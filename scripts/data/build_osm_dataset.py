...
This script implements the reproducible multi-layer pipeline:
  1. Reads raw OSM snapshots from data/raw/osm/thu_duc_osm_v1.0.0/.
  2. Generates intermediate cleaned topology files in data/interim/osm_thu_duc_v1.0.0/.
  3. Builds the standardized processed release in data/processed/osm_thu_duc_v1.0.0/.
"""
...
INTERIM_DIR = DATA_ROOT / "interim" / "osm_thu_duc_v1.0.0"
...
def main() -> None:
    print(f"Building OSM dataset release across layers: {DATASET_ID}...")
    ...
    # --- LAYER 2: INTERIM TOPOLOGY CLEANUP ---
    INTERIM_DIR.mkdir(parents=True, exist_ok=True)

    interim_nodes: list[dict[str, Any]] = []
    for node_id, node_data in sorted(raw_nodes.items()):
        tags = node_data.get("tags", {})
        interim_nodes.append({
    ...
    interim_nodes_path = INTERIM_DIR / "interim_nodes.csv"
    with interim_nodes_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["node_id", "osm_node_id", "name", "latitude", "longitude"])
        writer.writeheader()
        writer.writerows(interim_nodes)

    interim_edges: list[dict[str, Any]] = []
    for way in raw_ways:
        way_id = way["id"]
        tags = way.get("tags", {})
    ...
    interim_edges_path = INTERIM_DIR / "interim_edges.csv"
    with interim_edges_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(interim_edges[0].keys()))
        writer.writeheader()
        writer.writerows(interim_edges)

    print(f"Generated interim data layer: {len(interim_nodes)} nodes, {len(interim_edges)} edges in {INTERIM_DIR.relative_to(REPOSITORY_ROOT)}")

    # --- LAYER 3: PROCESSED RELEASE ---
    PROCESSED_DIR.mkdir(parents=True, exist_ok=True)

    processed_nodes = [
        {**node, "node_type": "intersection", "data_status": "SOURCE_BACKED"}
        for node in interim_nodes
    ]
    processed_edges = [
        {**edge, "data_status": "SOURCE_BACKED"}
        for edge in interim_edges
    ]
    ...
    # Write processed scenarios.json
    ...
    # Write processed delivery_points.csv
    ...
    print(f"Successfully built processed release {DATASET_ID}: {len(processed_nodes)} nodes, {len(processed_edges)} directed edges.")
