...
`POST /api/v1/alternatives` accepts the same start, goal, scenario and one selected
algorithm, then returns the primary route plus up to two distinct detours. Each
detour is generated by temporarily closing an edge from the previous route, so all
returned routes use the same algorithm and scenario and are ordered by weighted cost.
